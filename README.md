...
3. 画像を含める場合や、メタデータをファイルとして残す場合は上部の「⋯」→「記録先を選ぶ」からフォルダーを指定する
4. 作品ページでポップアップを開き、「この作品を記録」を選択する
5. 「記録一覧を開く」から記録した作品を閲覧する

> [!TIP]
> 記録一覧ページは、ブラウザでブックマークしておくとすぐに開けます。操作手順は上部の「⋯」→「使い方」からいつでも再表示できます。

### 記録一覧の閲覧

- よく使う検索・並び替え・表示切替は上部に表示しています。記録先・画像を含める設定・表示テーマ・使い方・ショートカット・外部リンクは「⋯」メニューにまとめています
...
- タイルをクリックすると元の縦横比でビューワーが開きます。タイル表示ではハートボタンを隠すため、お気に入りの変更は画像ビューワー、または通常表示で行います。選択・作品メニューは両モードで利用できます
- 選択チェックボックスはホバー時・キーボードフォーカス時に表示され、選択済みの作品では常時表示されます。タッチ端末では常時表示されます
- 表示モードは次回も保持されます。すべて選択は、未読み込み分も含めた絞り込み結果全体が対象です

### カレンダー（Beta）

- 一覧上部のカレンダーで、追加日（記録日）とイラスト投稿日を切り替えて月別の作品数を確認できます
- 日付の数字を囲む塗りつぶしの丸は、その月の最多件数を基準に、作品数が多いほど5段階で大きく濃くなります。丸は半透明で、0件の日は表示しません
- 矢印で前後の月へ移動し、年月入力で任意の月へ移動できます。「今月」で戻れます
- 日付を押すと一覧を絞り込みます。同じ日を再度押すか解除ボタンで戻ります。月や集計日を切り替えると日付の絞り込みを解除します
- 検索・タグ・お気に入りの条件を集計に反映します。複数ページの作品も1件として数え、日付不明の作品は別途件数を表示します。日時は端末のローカル時刻を使用します
- 通常は非表示です。並び替えボタンの右にあるSVGカレンダーボタンをクリックすると横に表示されます。再クリック・Escキー・外側のクリックで閉じられます。年月・左右矢印・今月ボタンは1行で操作できます。四角い枠は選択日を示し、詳しい説明は「?」から確認できます

### 画像ビューワー

- ビューワーは作品画像を中央に大きく表示し、右下のボタンでページや作品を移動できます
- 左下に作品タイトル・作成者・作品ID・元作品ページへのリンクを表示します
- 右下のハートで、表示中の作品をお気に入りに追加・解除できます。変更は一覧にも反映され、次回も保持されます（pixiv側のいいね・ブックマークは変更しません）
